...
func (s *MemeStore) EnsurePreviewAssets() error {
	...
	log.Printf("preview asset backfill: starting local-store video thumbnails for %d video(s)", totalVideos)
	...
	log.Printf(
		"preview asset backfill: finished local-store video thumbnails (total=%d generated=%d existing=%d failed=%d)",
		totalVideos,
		generated,
		existing,
		failed,
