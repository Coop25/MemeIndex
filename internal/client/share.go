...
func (s *Server) renderSharedMemePage(w http.ResponseWriter, r *http.Request, token string, meme accessor.Meme, expiresAt time.Time) {
	...
	mediaURL := origin + "/m/" + url.PathEscape(meme.ID) + "/media?share=" + url.QueryEscape(token)
	previewURL := origin + "/m/" + url.PathEscape(meme.ID) + "/preview?share=" + url.QueryEscape(token)
	...
	meta := `<meta property="og:image" content="` + html.EscapeString(origin+"/og-image.svg") + `">`
	switch {
	case strings.HasPrefix(meme.ContentType, "image/"):
		meta = `<meta property="og:image" content="` + html.EscapeString(mediaURL) + `"><meta property="og:image:type" content="` + html.EscapeString(meme.ContentType) + `">`
	...
	case strings.HasPrefix(meme.ContentType, "video/"):
		...
		if meme.PreviewPath != "" {
			meta += `<meta property="og:image" content="` + html.EscapeString(previewURL) + `">`
		}
